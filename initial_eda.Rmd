...
# Subset
olfaction_data <- subset(labels, Intensity == "high ", select = c("Compound.Identifier", "VALENCE.PLEASANTNESS"))
str(olfaction_data)
...
data.frame(cbind(identifiers, molec_disc_pca))
colnames(molec_disc_pca)[1] <- "CID" # <- rename to CID
...
joined <- merge(olfaction_data, molec_disc_pca, by = "CID")
joined[, -2]
head(joined)
